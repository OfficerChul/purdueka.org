...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const done = async () => {
    await _api.fleamarket.update(+params.id, {
      title,
      value,
    })
    router.push(`/fleamarket/${params.id}`)
  }
  const cancel = () => router.push(`/fleamarket/${params.id}`)
  if (isLoading) return <p>Loading...</p>
  if (!data) return <p>No data</p>

  return <div>
    <div>
      <div className='text-yellow-600 text-2xl mt-4'>글쓰기 - 사고팔고</div>
      <div className="mt-4">
