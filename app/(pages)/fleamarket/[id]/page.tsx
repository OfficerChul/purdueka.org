...
import { ReadOneFleamarketResponseDto } from "app/_dto/fleamarket.dto"
...
export default function Page({ params }: { params: { id: string } }) {
  const [data, setData] = useState({} as ReadOneFleamarketResponseDto)
  ...
  const submitComment = async () => {
    ...
    setComment("")
    _api.fleamarket.getOne(params.id)
      .then(e => setData(e))
  ...
  const handleRemove = () => {
    ...
    if (yes) {
      _api.fleamarket.remove(data.id)
  ...
  return (
    <div className="mt-10">
      ...
      <div className="flex justify-between">
        <div className='text-2xl'><Link href="/fleamarket">사고팔고</Link></div>
        {
          hasLoggedIn(user) &&
          data.authorId === user.id &&
          <div className='flex'>
            <button onClick={() => router.push(`/fleamarket/${params.id}/edit`)} className="bg-boilermaker-gold ms-4 py-2 px-4">수정하기</button>
